...
const ComingSoon = () => {
  ...
  const fetchComingSoonMovies = async () => {
    try {
      ...
      const response = await api.get('/movies?isComingSoon=true');
      
      if (response.data.success) {
        console.log('Coming soon movies data:', response.data.data);
        console.log('First movie poster:', response.data.data[0]?.poster);
      ...
      console.error('Fetch coming soon movies error:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-dark-950 pt-24 px-4 pb-12 flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-white text-lg">Loading coming soon movies...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-dark-950 pt-24 px-4 pb-12 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">No Coming Soon Movies</h2>
          <p className="text-dark-300 mb-6">Check back later for upcoming releases!</p>
          <Link 
            to="/movies"
  ...
  return (
    <div className="min-h-screen bg-dark-950 pt-24 px-4 pb-12">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        ...
        <p className="text-dark-300 text-lg">Get ready for the most anticipated upcoming releases</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {movies.map((movie) => (
            <div key={movie._id} className="glass rounded-xl overflow-hidden border border-dark-800 hover:border-cinema-600 transition-all duration-300 group">
              {/* Movie Poster */}
              <div className="relative w-full h-80 rounded-t-xl">
                <img 
                  src={movie.poster}
                  alt={movie.title}
                  className="w-full h-full object-cover"
                  onLoad={() => console.log(`Image loaded: ${movie.poster}`)}
                  onError={() => console.log(`Image failed: ${movie.poster}`)}
                />
                ...
                {/* Release Date Badge */}
                <div className="absolute bottom-4 left-4 bg-dark-900/90 backdrop-blur-sm text-white px-3 py-2 rounded-lg">
